...
namespace AmmPlayer.Extensions
{
    public class FolderBrowserDialogEx : CommonDialog
    {
        ...
        {
            this._uiFlags += BrowseFlags.BIF_BROWSEFORPRINTER;
        ...
        {
            this._uiFlags += BrowseFlags.BIF_BROWSEFORCOMPUTER;
        ...
        private int FolderBrowserCallback(IntPtr hwnd, int msg, IntPtr lParam, IntPtr lpData)
        {
            switch (msg)
            {
                ...
                case BrowseForFolderMessages.BFFM_SELCHANGED:
                    ...
                    if (pidl != IntPtr.Zero)
                    {
                        if (((this._uiFlags & BrowseFlags.BIF_BROWSEFORPRINTER) == BrowseFlags.BIF_BROWSEFORPRINTER) ||
                            ((this._uiFlags & BrowseFlags.BIF_BROWSEFORCOMPUTER) == BrowseFlags.BIF_BROWSEFORCOMPUTER))
                        {
                        ...
                        else
                        {
                            ...
                            if (haveValidPath && !String.IsNullOrEmpty(displayedPath))
                            {
                                if (this._showEditBox && this._showFullPathInEditBox)
                                {
                                    if (this._hwndEdit != IntPtr.Zero)
                                        PInvoke.User32.SetWindowText(this._hwndEdit, displayedPath);
                                }

                                if ((this._uiFlags & BrowseFlags.BIF_STATUSTEXT) == BrowseFlags.BIF_STATUSTEXT)
        ...
        protected override bool RunDialog(IntPtr hWndOwner)
        {
            ...
            if (this._dontIncludeNetworkFoldersBelowDomainLevel)
                this._uiFlags += BrowseFlags.BIF_DONTGOBELOWDOMAIN;
            if (this._newStyle)
                this._uiFlags += BrowseFlags.BIF_NEWDIALOGSTYLE;
            if (!this._showNewFolderButton)
                this._uiFlags += BrowseFlags.BIF_NONEWFOLDERBUTTON;
            if (this._showEditBox)
                this._uiFlags += BrowseFlags.BIF_EDITBOX;
            if (this._showBothFilesAndFolders)
                this._uiFlags += BrowseFlags.BIF_BROWSEINCLUDEFILES;
